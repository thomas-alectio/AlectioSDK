...
def test_anchors():
    configs = [{"width": 10, "height": 15, "x_stride": 8, "y_stride": 15}]
...
def test_hardnegative_multiboxesloss():
    ...
    configs = [
        {"width": 10, "height": 13, "x_stride": x_stride, "y_stride": y_stride},
        {"width": 16, "height": 30, "x_stride": x_stride, "y_stride": y_stride},
        {"width": 33, "height": 27, "x_stride": x_stride, "y_stride": y_stride},
    ]
    ...
    loss_fn(predicted_priors, predicted_objectness, predicted_class_dist, boxes, labels)


def test_mAP():
    # mAP should be 1 for perfect detection

    det_boxes = [cxcy_to_xy(F.softmax(torch.rand(10, 4), dim=1))]

    det_labels = [torch.LongTensor(range(10))]

    det_scores = [torch.ones(10, dtype=torch.float)]

    true_boxes = det_boxes
    true_labels = det_labels

    true_difficulties = [torch.zeros(10, dtype=torch.float)]
