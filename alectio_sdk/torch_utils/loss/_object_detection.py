...
class HardNegativeMultiBoxesLoss(nn.Module):
    ...
    def forward(
        self, predicted_boxes, predicted_objectness, predicted_class_dist, boxes, labels
    ):
        ...
        assert (
            n_priors
            == predicted_boxes.shape[1]
            == predicted_class_dist.shape[1]
            == predicted_objectness.shape[1]
        )
        ...
        true_locs = torch.zeros(
            (batch_size, n_priors, 4), dtype=torch.float, device=self.device
        )

        # true classes relative of each predicted_boxes
        true_classes = torch.zeros(
            (batch_size, n_priors), dtype=torch.long, device=self.device
        )
        ...
        for i in range(batch_size):
            ...
            iou_for_each_prior, object_for_each_prior = iou.max(
                dim=0
            )  # n_priors, n_priors
        ...
        loc_loss = F.smooth_l1_loss(
            predicted_boxes[positive_priors], true_locs[positive_priors]
        )
        ...
        true_objectness = torch.zeros_like(
            predicted_objectness, device=self.device, dtype=torch.float
        )
        ...
        hardness_ranks = (
            torch.LongTensor(range(n_priors), device=self.device)
            .unsqueeze(0)
            .expand_as(obj_loss_neg)
        )
