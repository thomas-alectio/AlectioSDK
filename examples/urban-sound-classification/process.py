...
def pre_process_fold(root, fold_name, class_to_label, meta, viz=False):
    ...
    for file in tqdm(files):
        try: 
            ...
            length = S_dB.shape[1] # the 4 second clip has length of 173 (trim anything with length greater than 173)
...
def create_10_fold_data(ROOT="./data/"):
    ...
    for fold in tqdm(folds):
        ...
        fold_data, fold_labels = pre_process_fold("./data/audio/", fold, class_to_label, meta, viz=False)
...
def processData(args, stageFor="train", indices=None):
    ...
    loader = DataLoader(dataset, batch_size=64, shuffle=True) #TODO: add arg for batch size
