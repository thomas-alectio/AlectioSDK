...
def train(args, labeled, resume_from, ckpt_file):
    ...
    output_directory = "{}/".format(args["EXPT_DIR"])
...
def test(args, ckpt_file):
    ...
    print("TEST SIZE: {}".format(test_image_count))
    ...
    return {"predictions": list(y_pred), "labels": list(y_true)}


def infer(args, unlabeled, ckpt_file):
    # Load the last best model

    output_directory = "{}/".format(args["EXPT_DIR"], FOLD)
    model = load_model(output_directory + ckpt_file)
    train_label_file = "{}train_subset{}.csv".format(args["LABEL_DIRECTORY"], FOLD)
    train_dataframe = pd.read_csv(train_label_file, dtype=str)
    train_dataframe["Absolutefilename"] = (
    ...
    test_image_count = currunlabelledtrain_dataframe.shape[0]
    ...
    preds = model.predict_generator(
        unlabelled_data_generator, test_image_count // args["BATCH_SIZE"] + 1
    )[: len(unlabeled)]

    print("PREDS: {}".format(len(preds)))

    predictions = {
        i: {"pre_softmax": list(row), "prediction": np.argmax(preds[i])}
...
def getdatasetstate(args, split="train"):
    if split == "train":
        train_label_file = "{}train_subset{}.csv".format(args["LABEL_DIRECTORY"], FOLD)
        dataframe = pd.read_csv(train_label_file, dtype=str)
        dataframe["Absolutefilename"] = args["IMG_DIRECTORY"] + dataframe["Filename"]
    else:
        test_label_file = "{}test_subset{}.csv".format(args["LABEL_DIRECTORY"], FOLD)
        dataframe = pd.read_csv(test_label_file, dtype=str)
        dataframe["Absolutefilename"] = args["IMG_DIRECTORY"] + dataframe["Filename"]
...
if __name__ == "__main__":
    ...
    infer(args, unlabeled=z, ckpt_file=ckpt_file)
    print("Train, Test, Infer complete!")
