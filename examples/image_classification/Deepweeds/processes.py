...
def train(args,labeled, resume_from, ckpt_file):
        ...
        train_dataframe['Absolutefilename'] = args['IMG_DIRECTORY'] + train_dataframe['Filename']
        val_dataframe = pd.read_csv(val_label_file)
        val_dataframe['Absolutefilename'] = args['IMG_DIRECTORY'] + val_dataframe['Filename']
        ...
        if args['model_name'] == "resnet":
            base_model = ResNet50(weights='imagenet', include_top=False, input_shape=args['INPUT_SHAPE'])
        elif args['model_name'] == "inception":
            base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=args['INPUT_SHAPE'])
        x = base_model.output
        ...
        model_checkpoint = ModelCheckpoint(output_directory + ckpt_file, verbose=1, save_best_only=True)
        early_stopping = EarlyStopping(patience=args['STOPPING_PATIENCE'], restore_best_weights=True)
        tensorboard = TensorBoard(log_dir=output_directory, histogram_freq=0, write_graph=True, write_images=False)
        reduce_lr = ReduceLROnPlateau('val_loss', factor=0.5, patience=args['LR_PATIENCE'], min_lr=0.000003125)
        model.compile(loss='binary_crossentropy', optimizer=Adam(lr=args['INITIAL_LR']), metrics=['categorical_accuracy'])
        ...
        while global_epoch < args['MAX_EPOCH']:
            ...
            if early_stopping.stopped_epoch == 0:
                ...
                global_epoch = global_epoch + early_stopping.stopped_epoch - args['STOPPING_PATIENCE'] + 1
                ...
                print("Restarting from last best val_loss at local epoch {} (global epoch {}).".format(
                    early_stopping.stopped_epoch - args['STOPPING_PATIENCE'], global_epoch - args['STOPPING_PATIENCE']))
                restarts = restarts + 1
                model.compile(loss='binary_crossentropy', optimizer=Adam(lr=args['INITIAL_LR'] / 2 ** restarts),
                              metrics=['categorical_accuracy'])
                model_checkpoint = ModelCheckpoint(output_directory + ckpt_file,
...
def test(args, ckpt_file):
        ...
        test_dataframe['Absolutefilename'] = args['IMG_DIRECTORY'] + test_dataframe['Filename']
        ...
        predictions = model.predict_generator(test_data_generator, test_image_count // args['BATCH_SIZE'] + 1)
...
def infer(args,unlabeled, ckpt_file):
    ...
    # No testing image augmentation (except for converting pixel values to floats)
    unlabelled_data_generator = ImageDataGenerator(rescale=1. / 255)
    # Load test images in batches from directory and apply rescaling
    ...
    # Evaluate model on test subset for kth fold
    preds = model.predict_generator(unlabelled_data_generator, test_image_count // BATCH_SIZE + 1)
