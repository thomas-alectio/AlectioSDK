    ...
    for epoch in range(args["train_epochs"]):
        for step in range(steps_per_epoch):
            ...
            if (step + 1) % args["DISPLAY_STEP"] == 0 or step == 0:
                ...
                print("Epoch:", '%03d' % (epoch + 1),"Step:", '%04d' % (step + 1), " loss=",
...
def test(args, ckpt_file):
    ...
    return {"predictions": y_pred.numpy().reshape(len(y_pred),1), "labels": y_true.numpy().reshape(len(y_true),1)}
...
def getdatasetstate(args, split="train"):
    ...
    for ix, row in dataframe.iterrrows():
        pathdict[ix] = row['Label']                                 ######### here index carries the necessary reference    
