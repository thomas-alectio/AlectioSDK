...
def train(args,labeled, resume_from, ckpt_file):
    ...
    traindataset = BasicDataset(args["TRAINIMAGEDATA_DIR"], args["TRAINLABEL_DIRECTORY"], img_scale)
    train = Subset(traindataset,labeled)
    valdataset = BasicDataset(args["VALIMAGEDATA_DIR"], args["VALLABEL_DIRECTORY"], img_scale)
    train_loader = DataLoader(train, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
    val_loader = DataLoader(valdataset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True, drop_last=True)
    global_step = 0
...
def test(args, ckpt_file):
    testdataset = BasicDataset(args["TESTIMAGEDATA_DIR"], args["TESTLABEL_DIRECTORY"], img_scale)
    val_loader = DataLoader(testdataset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True, drop_last=True)
    net = UNet(n_channels=3, n_classes=1, bilinear=True)
    net.to(device=device)
...
def infer(args,unlabeled, ckpt_file):
    ...
    unlabeled_loader = DataLoader(unlableddataset, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
    predix = 0
    predictions ={}
    net = UNet(n_channels=3, n_classes=1, bilinear=True)
    net.to(device=device)
